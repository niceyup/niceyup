...
import { defaultEmbeddingModels } from '@/lib/default-models'
...
export function EmbeddingModelSelect({
  ...
  const embeddingModels = React.useMemo(() => {
    ...
    return defaultEmbeddingModels[provider as ModelProvider] ?? []
  }, [provider])

  const filteredEmbeddingModels = React.useMemo(() => {
    if (!provider) {
      return []
    }
  ...
  }, [embeddingModels, inputValue])

  const shouldShowAddCustomModel = React.useMemo(() => {
    return Boolean(
      inputValue &&
        inputValue !== selectedOption &&
        !embeddingModels.find(({ value }) => value === inputValue),
    )
  }, [embeddingModels, inputValue, selectedOption])
  ...
  return (
    <Popover onOpenChange={setOpen} open={open}>
      <PopoverTrigger asChild>
        ...
        >
          {selectedOption
            ? (embeddingModels.find(({ value }) => value === selectedOption)
