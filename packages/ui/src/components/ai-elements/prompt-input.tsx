    ...
    <PromptInputHoverCard>
      <HoverCardTrigger asChild>
        ...
        >
          ...
          <span className="max-w-40 flex-1 truncate">{attachmentLabel}</span>
        </div>
      </HoverCardTrigger>
...
export function PromptInputAttachments({
  ...
  return (
    <div
      className={cn(
        'flex w-full flex-wrap items-center justify-start gap-2 p-3',
        className,
      )}
      {...props}
    >
...
> & {
  ...
  // Upload file to server and return metadata
  uploadFile?: (file: { id: string; file: File }) => Promise<
...
export const PromptInput = ({
  ...
  const addLocal = useCallback(
    ...
    [matchesAccept, maxFiles, maxFileSize, uploadFile, onError],
  )
